...
public class ConnectDialog extends JDialog implements ActionListener, ChangeListener {
	...
	private static final String EMUL_LOGLEVEL = "emul.logLevel";
	private static final String SS_LABELCONNECTIONS = "ss.labelConnections";
	private static final String SS_TITLE = "ss.title";
	private static final String EMUL_SHOWCONNECTDIALOG= "emul.showConnectDialog";
	private static final String EMUL_STARTLASTVIEW= "emul.startLastView";
	private static final String CHANGE_NONE_TO_TLS = "emul.change_none_to_tls";
	...
	private void jbInit() throws Exception {
		...
		//setResizable(false);
		//this.setDefaultCloseOperation(JDialog.); //did not seem to work, the JVM stayed acive
		...
		optionTabs.addTab(LangTool.getString(SS_LABELCONNECTIONS), sessionPanel);
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (!p.getTitleAt(index)
				.equals(LangTool.getString(SS_LABELCONNECTIONS))) {
			connectButton.setEnabled(false);
			this.setTitle(LangTool.getString(SS_TITLE) + " - "
					+ p.getTitleAt(index));
		} else {
			this.setTitle(LangTool.getString(SS_TITLE) + " - "
					+ LangTool.getString(SS_LABELCONNECTIONS));
	...
	private void createSessionsPanel() {
		...
		sessions.getActionMap().put(CONNECT, connectAction);
		...
		// Create the scroll pane and add the sessions table to it.
		...
		// add  option buttons to sessionOptPanel (first line of buttons)
		...
		// put all together into the session panel
		sessionPanel.setLayout(borderLayout);
		TitledBorder tb = BorderFactory.createTitledBorder("Session Configuration");
		tb.setTitleJustification(TitledBorder.CENTER);
		sessionPanel.setBorder(tb);

		addEnforceTlsOptionTo(sessionPanel, BorderLayout.NORTH);
	...
	private void createEmulatorOptionsPanel() {
		...
		if (properties.containsKey(EMUL_SHOWCONNECTDIALOG))
		...
		if (properties.containsKey(EMUL_STARTLASTVIEW))
	...
	private void createLoggingPanel() {
		...
		int logLevel = Integer.parseInt(properties.getProperty(EMUL_LOGLEVEL,
	...
	private void createButtonOptions() {
		...
		addOptButton(LangTool.getString("ss.optCancel"), CANCEL, options);
	}
	...
	private void cancel(){
		saveProps();
		setVisible(false);
		if(SessionManager.instance().getSessions().getSessionsList().isEmpty()){
			my_tn5250.exit(); //My5250 decides what to do
		}
	}
	...
	private void setLogLevel() {
		...
		if (intOFF.isSelected()) {
			...
			properties
			.setProperty(EMUL_LOGLEVEL, Integer
					.toString(TN5250jLogger.OFF));
		}

		if (intDEBUG.isSelected()) {

			properties.setProperty(EMUL_LOGLEVEL, Integer
	...
	private void showMe_itemStateChanged(ItemEvent e) {
		...
		if (showMe.isSelected()) {
			properties.setProperty(EMUL_SHOWCONNECTDIALOG, "");
		} else {

			properties.remove(EMUL_SHOWCONNECTDIALOG);
		}
	...
	private void lastView_itemStateChanged(ItemEvent e) {
		...
		if (lastView.isSelected()) {
			properties.setProperty(EMUL_STARTLASTVIEW, "");
		} else {

			properties.remove(EMUL_STARTLASTVIEW);
		}
	...
	public static void initialiseProperties(){
		//if no connect dialog pops up first, a session will be started, 
		//this session tries to access the TLS property (in case the encryption setting is NONE)
		//-> properties needed to be outsourced from ConnectDialog panel creation and configured before start
		properties = ConfigureFactory.getInstance().getProperties(ConfigureFactory.SESSIONS);
		createAndSetTlsCheckbox();
	}


	// TODO can My5250 use Singleton pattern without side effects?
	private static My5250 my_tn5250=null;
	public static void setMy5250( My5250 my5250){
		my_tn5250 = my5250;
	}
	...
	private static JCheckBox tlsAlwaysCB ;
	
	private static void createAndSetTlsCheckbox(){
		...
		tlsAlwaysCB.addItemListener(new java.awt.event.ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (tlsAlwaysCB.isSelected()){
					properties.setProperty(CHANGE_NONE_TO_TLS, "");
				}else{
					properties.remove(CHANGE_NONE_TO_TLS);
				}
			}
		});
	}
	
	private static void addEnforceTlsOptionTo(JPanel jp, String borderLayoutPosition) {
		jp.add(tlsAlwaysCB, borderLayoutPosition);
	}
